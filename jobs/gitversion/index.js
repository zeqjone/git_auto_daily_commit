...
class GitVersionJob {
  ...
  async makeVersionChange() {
    ...
    let [str, str2] = await Promise.all([this.getStrPackage(packagePath), this.getStrPackage(packagePath2)])
      .catch(err => {
        logger.error('read error', err);
      });
    await Promise.all([this.setPackage(packagePath, str), this.setPackage(packagePath2, str2)])
      .catch(err => {
